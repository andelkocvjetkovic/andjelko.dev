...
What is valid number, where is it come from? 
It is value that user can type in input field, or it can be a value that user tries to past from clipboard, 
or it can be a value that comes from the server.
